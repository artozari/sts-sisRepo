...
const CONFIGLOCAL: ConfigClass = new ConfigClass(); // --> cambiar por el que se use
...
setInterval(async () => {
    ...
    (async () => {
        if (CASINO_PUBLISHER.requestSync === 1) {
            const response = await HEALTH_CHECK.queryEndpoint("/api/v1/game?q=1000"); //--> consulta para obtener los últimos 1000 juegos, luego se puede cambiar por la cantidad que se quiera, modificando la API
            if (response.success) {
