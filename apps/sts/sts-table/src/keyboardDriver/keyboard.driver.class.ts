import { KeyboardDriverInterface } from "./interfaces";

export interface KeyboardDriverProcInterface {
  state: boolean;
  time: number;
}
 ...
 */
export class KeyboardDriverClass {
  private arrayKeys: KeyboardDriverProcInterface[] = [];
  /**
   * @constructor
   * @param {_keyQ} number - number of keys
   */
  constructor(private _keyQ: number) {
   ...
   */
  public procKeys(p_key: boolean[]): KeyboardDriverInterface[] {

    const resp: KeyboardDriverInterface[] = [];
    ...
    try {
      ...
      if (p_key.length > this.arrayKeys.length) {
        ...
        const arrayReFill: boolean[] = new Array(this.arrayKeys.length - p_key.length).fill(false);
      ...
      for (let i = 0; i < this._keyQ; i++) {
        if (this.arrayKeys[i].state !== p_key[i]) {
          ...
          const deltaT: number = now - this.arrayKeys[i].time;
