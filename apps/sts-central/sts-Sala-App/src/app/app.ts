...
export class App implements OnInit {
  ...
  obtenerTotalesTapete(allData: IRow[]) {
    ...
    const total = totalGames.cantidades.reduce((a, b) => a + b, 0);
    ...
    const col1Valor = totalGames.cantidades
      .filter((_, i) => col1.has(totalGames.ruleta[i]))
      .reduce((a, b) => a + b, 0);
    const col2Valor = totalGames.cantidades
      .filter((_, i) => col2.has(totalGames.ruleta[i]))
      .reduce((a, b) => a + b, 0);
    const col3Valor = totalGames.cantidades
      .filter((_, i) => col3.has(totalGames.ruleta[i]))
      .reduce((a, b) => a + b, 0);
    const totales: IRowTotals[] = [
      ...
      },
      {
        ...
        valor: col1Valor,
        porcentaje: total ? Math.round((col1Valor / total) * 100) : 0,
      },
      {
        tipo: 'segunda columna',
        valor: col2Valor,
        porcentaje: total ? Math.round((col2Valor / total) * 100) : 0,
      },
      {
        tipo: 'tercera columna',
        valor: col3Valor,
        porcentaje: total ? Math.round((col3Valor / total) * 100) : 0,
      },
      {
        tipo: 'Docena 1-12',
        valor: totalGames.cantidades
