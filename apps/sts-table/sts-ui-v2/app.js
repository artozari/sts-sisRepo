...
const DiasDeDatosRawProcesados = (rows) => {
    ...
    let minFecha = fechas[0];
    let maxFecha = fechas[0];
    for (const f of fechas) {
        if (f < minFecha) minFecha = f;
    ...
    return dias.map((date) => {
        ...
        let juegoFin = null;
        if (datos) {
            juegoIni = datos.juegos[0];
            juegoFin = datos.juegos[0];
            for (const juego of datos.juegos) {
                if (juego < juegoIni) juegoIni = juego;
                if (juego > juegoFin) juegoFin = juego;
            }
        }
        return {
            date,
            ...
            juegoIni, // Número de juego inicial del día
            juegoFin, // Número de juego final del día
        };
    });
};
